...
DT = 0.1  # time tick [s]
MAX_T = 0.65  # max prediction time [m]
MIN_T = 0.35  # min prediction time [m]
...
class Odom(object):
    def __init__(self):
        self.waypoints   = np.genfromtxt('./ros_wall_follower/scripts/csv/odom_data_frenet4.csv', delimiter=',')
...
class FrenetPurePursuit(PID, Odom, Steering):
    def __init__(self):
        ...
        rospy.Timer(rospy.Duration(self.dt), self.timerPurePursuitCallback)
    ...
    def laserCallback(self, msg):
        if(self.obstacles_ready == False):
            ...
            for i in self.laser_angle_range:
                ...
                if (distance <= 30.0): # modificar distancia horizonte
