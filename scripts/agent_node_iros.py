...
class Agent(object):
    ...
    def timer_callback(self, elapsed):
        ...
        # pid variables
        p = 0.045 * self.error #0.24
        d = 0.025 * (self.error - self.error_1) / self.dt #0.012
        i = 0.001 * (self.error + self.error_1) * self.dt #0.012
        self.pid = p + i + d

        # linearization of the velocity according to the pid
        v = 7.0 - abs(self.pid) * 4.0/1.22
    ...
    def scan_callback(self, scan_msg):
        ...
        r_segment = np.array(scan_msg.ranges[100:540]) #140
        l_segment = np.array(scan_msg.ranges[-540:-100])
    ...
    def odom_callback(self, odom_msg):
        ...
        # np.savetxt('/home/rolando/catkin_ws/src/f1tenth_gym_ros/scripts/odom_data.dat', self.odom_list)
